...
export class LocationMapComponent {
  ...
  // trebamo instancu mape koje ce biti tipa LeafLet koji smo importali
  ...
  @Output()
  locationChangeEvent = new EventEmitter<string>();
  ...
  // kada mapa bude spremna biti cemo notoficirani ovom metoduom
  ...
  onLocationChange(event: AutoCompleteSelectEvent): void {
    const newCountry = event.value as Country;
    this.locationChangeEvent.emit(newCountry.cca3)
  }

  /* Prvo zelimo imati podatke koji dolaze iz coubtryService
     onda zelimo susati kada se signal promjenio
   */
  ...
  private changeMapLocation(term: string) {
    // extrat current location zemlje
  ...
  // search metod to bind it to autocompletion
