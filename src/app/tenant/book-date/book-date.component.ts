...
export class BookDateComponent implements OnInit, OnDestroy {
  ...
  // sada cemo listen metode koju cemo kreirati
  constructor() {
  ...
  public validateMakeBooking() {
    return this.bookingDates.length == 2
    && this.bookingDates[0] !== null && this.bookingDates[1]!== null
    && this.bookingDates[0].getTime() !== this.bookingDates[1].getDate()
    ...
    effect(() => {
      ...
      if(checkAvailabilityState.status === "OK"){
        ...
        this.toastService.send({
          severity: 'error', summary:"Error", detail:"Error listening to checkAvailable dates"
        });
      }
    });
  }
  ...
  private listenToCreateBooking() {
    effect(() => {
      ...
      if(createBookingState.status === "OK"){
        this.toastService.send({
          severity: 'success', summary: "Success", detail: "Booking created successfully",
        });
        this.router.navigate(['/booking']);
      }else if(createBookingState.status === "ERROR"){
        this.toastService.send({
          severity: 'error', summary: "Error", detail: "Error while creating booking",
        });
      }
